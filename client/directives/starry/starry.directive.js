...
let draggable = function(dom, config) {
  let top, left;
  let start = [];
  listenerManager.addEventListener(dom, 'ondragstart', (e) => {
    start = [ e.x, e.y ];
    top = parseInt(dom.style.top);
    left = parseInt(dom.style.left);
  });
  listenerManager.addEventListener(dom, 'ondrag', (e) => {
    let style = {};
    style.top = (top + e.y - start[1]) + 'px';
    style.left = (left + e.x - start[0]) + 'px';
    assignStyle(dom, style);
  });
  listenerManager.addEventListener(dom, 'ondragend', (e) => {
    let style = {};
    style.top = (top + e.y - start[1]) + 'px';
    style.left = (left + e.x - start[0]) + 'px';
    assignStyle(dom, style);
...
let addCanvas = function(root, config) {
  ...
  listenerManager.addEventListener(canvas.node(), 'ondblclick', () => { 
    canvas.remove(); 
    listenerManager.deleteDom(canvas.node());
...
export default angular.module('directive.starry', [])
  .directive('starry', [ 'event', '$timeout', '$http', function(event, $timeout, $http) {
    return {
      restrict: 'E',
      template: require('./starry.html'),
      link: function(scope, element, attrs) {
        ...
        let exportCallback = function(msg) {
          ...
          if (msg.dblclickword) {
            return $http({
              ...
              event.emit(event.SHOWRECONSTRUCT, {children, constructors, word});
              return data;
            });
          }
        ...
        assignStyle(backcanvas.node(), { backgroundColor: '#000' });
        assignStyle(forecanvas.node(), { backgroundColor: 'rgb(0,0,0,0)' });
        assignStyle(svg.node(), { backgroundColor: 'rgb(0,0,0,0)' });
        // Assign width and height of canvas
        assignAttr(backcanvas.node(), { width: backcanvas.node().clientWidth, height: backcanvas.node().clientHeight });
        assignAttr(forecanvas.node(), { width: forecanvas.node().clientWidth, height: forecanvas.node().clientHeight });
        assignAttr(svg.node(), { width: svg.node().clientWidth, height: svg.node().clientHeight });
        // New data
        ...
        let emitter = (d) => {
          ...
          $http({
            ...
            let id = data.id;
            event.emit(event.SHOWIMAGECONSTRUCTOR, {constructors, id});
            svg.selectAll('.children').classed('children', false);
            svg.selectAll('.parent').classed('parent', false);
            let constructor_words = constructors.map(d => util.getWordByIndex(d.index));
            constructor_words = svg.selectAll('.keyword')
              .filter(d => {
            ...
            constructor_words.classed('parent', true);
            return msg;
          });
        };
